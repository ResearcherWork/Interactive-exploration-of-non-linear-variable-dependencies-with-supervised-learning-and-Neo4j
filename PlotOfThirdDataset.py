...
DF3=pd.read_csv('C:/PythonProjects/AgentsTurboFan/Test/Test3/Scores.csv', delimiter=",", names=["Nodes", "Correlation", "RAE", "RSE"])
result3 = DF3.sort_values(by=['Correlation'])
result3.to_csv('C:/PythonProjects/AgentsTurboFan/Test/Test3/ScoresSorted.csv', index=False)
DF3S=pd.read_csv('C:/PythonProjects/AgentsTurboFan/Test/Test3/ScoresSorted.csv', delimiter=",")
LN = []
for i in range(len(DF3S)):
    LN.append(DF3S['Nodes'][i])
LC = []
for i in range(len(DF3S)):
    LC.append(DF3S['Correlation'][i])
LRAE = []
for i in range(len(DF3S)):
    LRAE.append(DF3S['RAE'][i])
LRSE = []
for i in range(len(DF3S)):
    LRSE.append(DF3S['RSE'][i])
...
plt.title('Scatter Plot of the third data set')
...
plt.show()
fig.savefig("C:/PythonProjects/AgentsTurboFan/Test/Test3/Plot.pdf", bbox_inches='tight')